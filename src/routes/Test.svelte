<script>
  // @ts-nocheck
      ...
      function update(STEPS){
        ...
        let steps = (Array(STEPS + 1).fill(0))
        ...
        return [steps, renglones]
  
      }
      
      const generateExcel = async () => {
  
        let [pasitos, suavecitos] = update(STEPS);
        ...
        worksheet.columns = pasitos;
  
        suavecitos.forEach(suave => {
          worksheet.addRow(suave);
        });
    ...
    <Button  on:click={generateExcel} class="mx-auto">
      Download Excel
    </Button>
